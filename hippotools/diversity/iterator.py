...
from dexom_python.model_functions import (
    load_reaction_weights,
    read_model,
    check_model_options,
    check_threshold_tolerance,
)
from dexom_python.enum_functions.enumeration import (
    EnumSolution,
    create_enum_variables,
    read_prev_sol,
)
from dexom_python.enum_functions.icut_functions import create_icut_constraint
from dexom_python.enum_functions.maxdist_functions import (
    create_maxdist_constraint,
...
class DiversityEnumIterator:
    ...
    def __init__(
        ...
        prev_sol: cobra.Solution = None,
        epsilon: float = DEFAULT_VALUES["epsilon"],
        threshold: float = DEFAULT_VALUES["threshold"],
        obj_tol: float = DEFAULT_VALUES["obj_tol"],
        maxiter: int = DEFAULT_VALUES["maxiter"],
        dist_anneal: float = DEFAULT_VALUES["dist_anneal"],
        out_path: TextIO = "enum_dexom",
        icut: bool = True,
        full: bool = False,
        save: bool = False,
        verbose=True,
        ...
        check_threshold_tolerance(
            model=self.model, epsilon=self.eps, threshold=self.thr
        )
    ...
    def __iter__(self):
        ...
        if self.prev_sol is None:
            ...
            self.model = create_enum_variables(
                model=self.model,
        ...
        self.prev_sol_bin = (
            np.abs(self.prev_sol.fluxes) >= self.thr - self.tol
        ).values.astype(int)
        ...
        self.opt_const = create_maxdist_constraint(
            self.model,
            self.reaction_weights,
            self.prev_sol,
            self.obj_tol,
    ...
    def __next__(self):
        ...
        if self.iter > self.maxiter:
            ...
            self.model.solver.remove(
                [
                    const
                    for const in self.icut_constraints
        ...
        if self.icut:
            const = create_icut_constraint(
                self.model,
                reaction_weights=self.reaction_weights,
                threshold=self.thr,
                prev_sol=self.prev_sol,
                name="icut_" + str(self.iter),
                full=self.full,
            )
        ...
        for rid, weight in six.iteritems(self.reaction_weights):
            if np.random.random() > self.dist_anneal**self.iter and weight != 0:
                ...
                i += 1
        objective = create_maxdist_objective(
            self.model, tempweights, self.prev_sol, self.prev_sol_bin, full=self.full
        )
        self.model.objective = objective
        t2 = time.perf_counter()
        if self.verbose:
            print(
                "Time before optimizing in iteration " + str(self.iter) + ":", t2 - t0
            )
        with catch_warnings():
            ...
            try:
                ...
                if self.save:
                    ...
                    if self.verbose:
                        print(
                            "time for optimizing in iteration " + str(self.iter) + ":",
                            t1 - t2,
                        )
                ...
                if "time_limit" in str(w):
                    print(
                        "The solver has reached the timelimit in iteration %i. If this happens frequently, there may "
                        "be too many constraints in the model. Alternatively, you can try modifying solver "
                        "parameters such as the feasibility tolerance or the MIP gap tolerance."
                        % self.iter
                    )
                    warn('Solver status is "time_limit" in iterations %i' % self.iter)
                elif "infeasible" in str(w):
                    print(
                        "The solver has encountered an infeasible optimization in iteration %i. If this happens "
                    ...
                    print(
                        "An unexpected error has occurred during the solver call in iteration %i."
                        % self.iter
                    )
                    warn(w)
        ...
        solution = EnumIterSolution(
            self.prev_sol, self.prev_sol_bin, self.prev_sol.objective_value, error
        )
        stats = {"selected_reactions": i, "time": t1 - t0}
...
class IcutEnumIterator:
    ...
    def __init__(
        ...
        model: cobra.Model,
        reaction_weights: pd.Series,
        prev_sol: Union[cobra.Solution, None] = None,
        epsilon: float = DEFAULT_VALUES["epsilon"],
        threshold: float = DEFAULT_VALUES["threshold"],
        obj_tol: float = DEFAULT_VALUES["obj_tol"],
        maxiter: int = DEFAULT_VALUES["maxiter"],
    ...
    def __next__(self):
        ...
        solution = EnumIterSolution(
            self.prev_sol, self.prev_sol_binary, self.objective_value, error
        )
        if self.verbose:
