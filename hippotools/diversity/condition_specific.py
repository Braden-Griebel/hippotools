...
def enforce_off(
    model: cobra.Model, solution: cobra.Solution, thr: float
) -> cobra.Model:
    """
...
def enforce_inactive(
    model: cobra.Model, solution: cobra.Solution, epsilon: float, low_expr_rxns: list
):
    """
...
def enforce_active(
    model: cobra.Model, solution: cobra.Solution, epsilon: float, high_expr_rxns: list
):
    """
...
def _force_inactive(
    epsilon: float, lb: float, ub: float, reversible: bool
) -> tuple[float, float]:
    """
    ...
    if reversible:
        ...
        if lb < 0:
            ...
            if lb <= -epsilon <= ub:
                ...
                raise ValueError(
                    "The upperbound is negative and greater than negative epsilon"
                )
            else:
...
def _force_active(
    epsilon: float, lb: float, ub: float, reversible: bool, forward: bool
) -> tuple[float, float]:
    """
    ...
    if reversible:
        if forward:
            if epsilon <= ub:
                ...
                raise ValueError(
                    "The reaction is reversible and positive epsilon is greater than the upper bound"
                )
        else:
            if -epsilon >= lb:
                updated_ub = -epsilon
                ...
                raise ValueError(
                    "The reaction is reversible and negative epsilon is less than the lower bounds"
                )
    else:
        if ub > 0:
            ...
            if lb <= epsilon <= ub:
                ...
                raise ValueError(
                    "The reaction is forward, but epsilon is greater than the upper bound"
                )
            else:
        ...
        if lb < 0:
            ...
            if lb <= -epsilon <= ub:
                ...
                raise ValueError(
                    "The reaction is reverse, but epsilon is less than the lower bound"
                )
            else:
