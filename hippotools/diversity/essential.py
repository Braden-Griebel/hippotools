...
# Local Imports
import hippotools.diversity.utils
from hippotools.diversity.condition_specific import (
    EnforceBoth,
    EnforceOff,
    EnforceActive,
    EnforceInactive,
...
from hippotools.seq_utils import expression_to_weights
...
def essential_genes(
    model: cobra.Model, essential_prop: float, return_error_status_genes: bool
) -> Union[set, tuple[set, set]]:
    """
    ...
    :param essential_prop: Cutoff for proportion of optimal growth achievable after gene knockout, if the optimal
        flux after a gene knockout is less than essential_prop*optimal_growth, the gene is considered essential
...
def consensus_essential_rxn_weights(
    ...
    **kwargs
    ...
    :param kwargs: Arguments to pass to iterators class constructor method
    :type kwargs: dicts
    :return: Dataframe of essentiality predictions, True means essential, False means non-essential, and pd.NA means
        that the solver returned an error. Index is genes, columns are iter_1, iter_2, etc. representing iterations.
    :rtype: pd.DataFrame
    """
    ...
    iterator = hippotools.diversity.utils.create_iterator(
        model=model,
        reaction_weights=reaction_weights,
        enum_method=enum_method,
        kwargs=kwargs,
    )
    context_method = hippotools.diversity.utils.parse_model_method(
        context_specific_method
    )
    low_expr_rxns = list(reaction_weights[np.isclose(reaction_weights, -1.0)].index)
    high_expr_rxns = list(reaction_weights[np.isclose(reaction_weights, 1.0)].index)
    ...
    for i, (solution, stats) in enumerate(iterator):
        ...
        if context_method == "enforce_off":
            ...
            raise ValueError(
                "Couldn't parse context specific method: %s" % context_specific_method
            )
        with manager as context_model:
            ess_genes, error_genes = essential_genes(
                context_model,
                essential_prop=essential_prop,
...
def consensus_essential_gene_expr(
    model: cobra.Model,
    essential_prop: float,
    gene_expression: Union[pd.DataFrame, pd.Series],
    gene_axis: int = 1,
    proportion: Union[float, tuple] = 0.25,
    ...
    **kwargs
    ...
    :rtype: pd.DataFrame
    """
    rxn_weights = expression_to_weights(
        gene_expression=gene_expression,
        proportion=proportion,
        gene_axis=gene_axis,
        agg_fun=agg_fun,
    )
    return consensus_essential_rxn_weights(
        model=model,
        essential_prop=essential_prop,
        reaction_weights=rxn_weights,
        enum_method=enum_method,
        context_specific_method=context_specific_method,
        **kwargs
    )
...
def compute_essentiality(
    consensus_essentiality_df: pd.DataFrame,
    aggregation_func: Callable[[pd.Series], bool],
    **kwargs
):
    """
    Computes essentiality predictions from the consensus_essentiality dataframe

    :param consensus_essentiality_df: Dataframe of essentiality predictions, True means essential, False means
        non-essential, and pd.NA means that the solver returned an error. Index is genes, columns are iter_1,
        iter_2, etc. representing iterations.
    :type consensus_essentiality_df: pd.DataFrame
    :param aggregation_func: Function to use for aggregating the essentiality predictions, should take a pd.Series
        of True, False, and NA and return a single boolean (or NA)
    :type aggregation_func: Callable[[pd.Series], bool]
    :return: Series of essentiality predictions, True means essential, False means non-essential, and pd.NA means some
        solver error
    :rtype: pd.Series
    """
    return consensus_essentiality_df.agg(aggregation_func, axis=1, **kwargs)
