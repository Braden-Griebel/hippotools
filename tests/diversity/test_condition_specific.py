...
# Local imports
from hippotools.diversity.condition_specific import _force_inactive, _force_active
...
class TestHelperFunctions(unittest.TestCase):
    ...
    def test_force_inactive(self):
        ...
        """
        self.assertEqual(
            _force_inactive(epsilon=1, lb=-2, ub=2, reversible=True), (-1, 1)
        )
        self.assertEqual(
            _force_inactive(epsilon=2, lb=-1, ub=1, reversible=True), (-1, 1)
        )
        self.assertEqual(
            _force_inactive(epsilon=1, lb=0, ub=2, reversible=False), (0, 1)
        )
        self.assertEqual(
            _force_inactive(epsilon=2, lb=0, ub=1, reversible=False), (0, 1)
        )
        self.assertEqual(
            _force_inactive(epsilon=1, lb=-2, ub=0, reversible=False), (-1, 0)
        )
        self.assertEqual(
            _force_inactive(epsilon=2, lb=-1, ub=0, reversible=False), (-1, 0)
        )
    ...
    def test_force_active(self):
        ...
        """
        self.assertEqual(
            _force_active(epsilon=1, lb=-2, ub=2, reversible=True, forward=True), (1, 2)
        )
        self.assertEqual(
            _force_active(epsilon=1, lb=2, ub=4, reversible=False, forward=True), (2, 4)
        )
        self.assertEqual(
            _force_active(epsilon=1, lb=-4, ub=-2, reversible=False, forward=False),
            (-4, -2),
        )
        self.assertEqual(
            _force_active(epsilon=4, lb=3, ub=5, reversible=False, forward=True), (4, 5)
        )
        self.assertEqual(
            _force_active(epsilon=4, lb=-5, ub=-3, reversible=False, forward=False),
            (-5, -4),
        )
        self.assertEqual(
            _force_active(epsilon=1, lb=-2, ub=2, reversible=True, forward=False),
            (-2, -1),
        )
        with self.assertRaises(ValueError):
...
class TestConditionSpecificFunctions(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        for i in range(1, 11):
            met = Metabolite(
                "M%i_c" % i,
                formula="C%iH%iN%i" % (i, 2 * i + 1, 1),
                name="Metabolite %i" % i,
                compartment="c",
            )
            metabolite_list.append(met)
        met1_ext = Metabolite(
            "M1_e", formula="C1H3N1", name="Metabolite 1 ext", compartment="e"
        )
        R1 = Reaction(
            "R1",
            name="Reaction 1",
        ...
        R6 = Reaction(
            "R6", name="Reaction 6", subsystem="boundary", lower_bound=-5, upper_bound=5
        )
        R6.add_metabolites({met1_ext: -1, metabolite_list[1]: 1})
